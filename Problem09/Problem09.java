...
public class Problem09 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int[] coords : lowPoints) {
      basinSizes.add( basinSize(map, coords, alreadyCounted) );
    }
  ...
  private static int basinSize(int[][] map, int[] coords, ArrayList<int[]> alreadyCounted) {
    int size = 0;

    if (!arrayListHasCoords(alreadyCounted, coords)) { // check that coords hasn't already been visited
      if (map[coords[0]][coords[1]] != 9) {
        // add to basin size, then mark current coordinates as visited
        ...
        for (int[] adjacent : adjacentCoordsList) {
          size += basinSize(map, adjacent, alreadyCounted);
        }
